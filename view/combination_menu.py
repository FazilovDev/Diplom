...
from models.algorithms.ast_clones import get_plag_combination

class CombinationMenu(Frame):
    ...
    def analyzer_several_files(self):
        files = get_files_in_directories(self.directory, '.py')
        results = []
        if self.full_info.get() == 0:
            for i in range(len(files)):
                for j in range(len(files)):
                    if i != j:
                        res = get_plag_combination(files[i], files[j], self.k.get(), self.q.get(), self.w.get(), self.k.get()+2, self.q.get(), self.w.get()-1)
                        if res != None:
                            results.append([files[i], files[j], res[1]])
        else:
            for i in range(len(files)):
                max_plag = 0
                max_file = ''
                for j in range(len(files)):
                    if i != j:
                        res = get_plag_combination(files[i], files[j], self.k.get(), self.q.get(), self.w.get(), self.k.get()+2, self.q.get(), self.w.get()-1)
                        if res != None and res[1] > max_plag:
                            max_plag = res[1]
                            max_file = files[j]
    ...
    def show_group_analyze(self):
        ...
        r1 = Radiobutton(frame4, text='полная информация',
                        variable=self.full_info, value=0)
        r1.pack()

        r2 = Radiobutton(frame4, text='краткая инфорация',
                        variable=self.full_info, value=1)
        r2.pack()
        self.text_k = Label(frame4, text="Размер граммы k:")
        self.text_k.config(bg="white")
        self.text_k.pack()
